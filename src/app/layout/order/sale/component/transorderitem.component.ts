...
@Component({
    selector: 'app-order-trans-list-item',
    templateUrl: './transorderitem.component.html'
})
export class TransOrderItemComponent implements OnInit {
    ...
    //构造
    constructor(private fb: FormBuilder,private location: Location, private route: ActivatedRoute,private warehouseService: WarehouseService,private router: Router,private transitOrderService:TransitOrderService,private authService:AuthService) {
    ...
    paginateQuery(event) {
        ...
        this.transitOrderService.pageQuery(params).subscribe(res=>{
            if(res.code == 0){
