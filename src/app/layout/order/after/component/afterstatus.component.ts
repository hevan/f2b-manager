...
@Component({
    selector: 'app-order-after-status',
    templateUrl: './afterstatus.component.html'
})
export class AfterStatusComponent implements OnInit {

    public listData = []; //定义form

    public pageNav = new PageDataModel();

    public searchData = {orderNo:'','userMobile':'',refundNo:'', 'status':'3','startDate':'','endDate':''};
    ...
    paginateQuery(event) {
        ...
        const params = new HttpParams().set('orderNo', this.searchData.orderNo).set('userMobile', this.searchData.userMobile).set('corpId',this.corp.id).set('status',this.searchData.status)
            .set('refundNo', this.searchData.refundNo)
        ...
        this.afterSaleService.pageQueryEx(params).subscribe(res=>{
            if(res.code == 0){
