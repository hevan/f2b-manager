...
@Component({
    selector: 'app-puch-salepurchaselist',
    templateUrl: './salepurchaselist.component.html',
    animations: [routerTransition()]
})
export class SalePurchaseListComponent implements OnInit {
    ...
    //构造
    constructor(private fb: FormBuilder,private location: Location, private route: ActivatedRoute,private router: Router,private constCodeService: ConstCodeService,private purchaseService:PurchaseService,private authService:AuthService) {
    ...
    paginateQuery(event) {
        ...
        const params = new HttpParams().set('billCode', this.searchData.billCode).set('billType',this.searchData.billType).set('supplyCorpId',this.corp.id).set('name',this.searchData.name).set('supplyCorpName',this.searchData.supplyCorpName).set('status',this.searchData.status)
    ...
    toView(id){
        this.router.navigate(['/sale/salepurchase/salepurchaseview', { 'id': id}]);
    }

    toEdit(id){
        this.router.navigate(['/sale/salepurchase/salepurchaseedit', { 'id': id}]);
    }
