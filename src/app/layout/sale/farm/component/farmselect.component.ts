...
@Component({
    selector: 'app-puch-farmselect',
    templateUrl: './farmselect.component.html'
})
export class FarmSelectComponent implements OnInit {
    ...
    //构造
    constructor(private fb: FormBuilder,private location: Location, private route: ActivatedRoute,private router: Router,private constCodeService: ConstCodeService,private farmPurchaseService:FarmPurchaseService,private authService:AuthService,public activeModal: NgbActiveModal) {
    ...
    paginateQuery(event) {
        ...
        const params = new HttpParams().set('billCode', this.searchData.billCode).set('supplyCorpId',this.corp.id).set('billType',this.searchData.billType).set('name',this.searchData.name).set('supplyCorpName',this.searchData.supplyCorpName).set('status',this.searchData.status)
