...
@Component({
    selector: 'app-puch-recvtransitlist',
    templateUrl: './recvtransitlist.component.html',
    animations: [routerTransition()]
})
export class RecvTransitBillListComponent implements OnInit {
    ...
    paginateQuery(event) {
        ...
        const params = new HttpParams().set('billCode', this.searchData.billCode).set('purchCorpId',this.corp.id).set('billType',this.searchData.billType).set('name',this.searchData.name).set('supplyCorpName',this.searchData.supplyCorpName).set('status',this.searchData.status)
    ...
    toView(id){
        this.router.navigate(['/puch/purchase/purchaseview', { 'id': id}]);
    }

    toEdit(id){
        this.router.navigate(['/puch/purchase/purchaseedit', { 'id': id}]);
    }
