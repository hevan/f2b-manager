...
@Component({
    selector: 'app-stock-enterstockedit',
    templateUrl: './enterstockedit.component.html',
    animations: [routerTransition()]
})
export class EnterStockEditComponent implements OnInit {
    ...
    //构造
    constructor(private location: Location, private route: ActivatedRoute,private router: Router,private enterStockService: EnterStockService,private constCodeService:ConstCodeService,private enterStockItemService:EnterStockItemService,private productSkuService:ProductSkuService,private productService:ProductService,private authService:AuthService,private constService:ConstService, private modalService: NgbModal) {
        this.createForm();
    }
    ...
    ngOnInit() {
        ...
        let params = new HttpParams().set('code', 'pt_enter_stock');
    ...
    onSubmit() {
        ...
        formData.purchCorpId = this.corp.id;
        formData.purchCorpName= this.corp.name;


        //contract.signedDate = JSON.stringify(contract.signedDate).substr(1,10);

        this.enterStockService.save(JSON.stringify(formData)).subscribe(res=>{
            if(res.code == 0){
    ...
    saveProductItem(){
        ...
        this.enterStockItemService.saveList(JSON.stringify(this.listPurchaseItemUpdate)).subscribe(res=>{
            if(res.code == 0){
                this.loadItem();
    ...
    genBillCode(){
        ...
        this.constService.genBillNo('E').subscribe(res=>{
           if(res.code ===0 ){
