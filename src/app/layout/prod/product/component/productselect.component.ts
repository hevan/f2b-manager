...
@Component({
    selector: 'app-prod-productselect',
...
export class ProductSelectComponent implements OnInit {
    ...
    //构造
    constructor(private fb: FormBuilder,private location: Location, private route: ActivatedRoute,private router: Router,private productService:ProductService,private categoryService: CategoryService,private brandService:BrandService,private authService:AuthService,public activeModal: NgbActiveModal) {
    ...
    paginateQuery(event) {
        ...
        const params = new HttpParams().set('category.id', this.searchData.category.id).set('brand.id',this.searchData.brand.id).set('name',this.searchData.name).set('code',this.searchData.code).set('page',curPage).set('itemsPerPage',this.pageNav.itemsPerPage+'');
    ...
    closeDialog(){
        ...
        if (this.selectProduct) {
            this.activeModal.close( JSON.stringify(this.selectProduct));
        }else{
